...
# defines tools and database sources
anno=/tools/snpEff
...
# in essence, it merges targets and removes all but first name  (; is default delimiter)
bedtools merge -nms -i nonmerged_targets.bed | sed 's/;.*//' > merged_targets.tmp # remove ;.*
# -nms Reporting the names of the features that were merged, deprecated since v. 2.20.0, see -c and -o
#      https://github.com/arq5x/bedtools2/blob/69eb4eaad5c9348f56812afed1e73eb57139f828/docs/content/tools/merge.rst#-nms-reporting-the-names-of-the-features-that-were-merged
# our approach should probably be "-c 4 -o collapse"
...
# finds minimum intersect between A and B, see link
# https://bedtools.readthedocs.io/en/latest/content/tools/intersect.html?highlight=intersect#default-behavior
bedtools intersect -a nonmerged_targets.bed -b nonmerged_targets.bed | awk 'a[$1FS$2FS$3]++' OFS="\t" > overlapped_regions.bed

bedtools subtract -a nonmerged_targets.bed -b overlapped_regions.bed > nonmerged_noolaps_targets.tmp
...
lofreq call --call-indels -f $REF $OUT.preMID.bam -l $BED -o $OUT.preMID.noPclip.lf.vcf # doesn't appear anywhere else
...
#Group Reads by UMI
java -jar $fgbio GroupReadsByUmi -s adjacency --edits 1 -i $OUT.sanitised.setmateinfo.qsort.bam -o $OUT.groupdbyMID.bam

#Make consensus N molecules to make consensus Out is coordinate sorted
java -jar $fgbio CallMolecularConsensusReads -M 3 -i $OUT.groupdbyMID.bam -o $OUT.M3.consensus.bam -r $OUT.M3.notused4consensus.bam
# -M 3 (The minimum number of reads to produce a consensus base.)
java -jar $fgbio CallMolecularConsensusReads -i $OUT.groupdbyMID.bam -o $OUT.consensus.bam -r $OUT.notused4consensus.bam
...
$GATK  -T PrintReads -R $REF -I ${OUT}.realigned.bam -BQSR ${OUT}.recal_data.table -o ${OUT}.fgbio.bqsrCal.bam
...
/seq/tools/lofreq_star-2.1.2/lofreq2_indel_ovlp.py $OUT.fgbio.bqsr.lf.vcf > $OUT.fgbio.bqsr.lf.mergeIndels.vcf
/seq/tools/lofreq_star-2.1.2/lofreq2_indel_ovlp.py $OUT.preMID.bqsr.lf.vcf > $OUT.preMID.bqsr.lf.mergeIndels.vcf
